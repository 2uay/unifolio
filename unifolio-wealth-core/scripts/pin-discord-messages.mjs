...
const messages = {
    ...
    embeds: [
      {
        ...
        footer: { text: 'unifolio.pro  ·  Built for serious investors' },
  ...
  'roadmap': {
    embeds: [
      ...
      {
        color: GOLD,
        title: '◆  Phase 2 — Connectivity  ·  IN PROGRESS',
    ...
    embeds: [
      {
        ...
        footer: { text: 'unifolio.pro is deployed on Vercel — 99.9% uptime SLA' },
      },
    ],
  },
    ...
    embeds: [
      {
        ...
        footer: { text: 'Full history in #roadmap  ·  unifolio.pro' },
      },
    ],
  },
    ...
    embeds: [
      {
        ...
        fields: [
          {
            name: 'How to write a good request',
            value: '```\n1. What problem does it solve?\n2. How do you currently work around it?\n3. What would the ideal solution look like?\n4. How often would you use it?\n```',
          },
          {
            name: 'What gets prioritized',
            value: 'Requests that align with Phase 2/3 of the roadmap, have multiple upvotes, and have a clear use case. Niche edge cases are logged but lower priority.',
          },
          {
            name: 'Already planned?',
            value: 'Check `#roadmap` first. If your idea is already there, react to this message instead of creating a duplicate.',
          },
        ],
    ...
    embeds: [
      {
        ...
        fields: [
          { name: 'Is Unifolio free?', value: 'Yes — the core product is free. A Pro tier is coming with advanced features. Beta testers get Pro free during testing.', inline: false },
    ...
    embeds: [
      {
        ...
        fields: [
          {
            name: 'Before you ask',
            value: '1. Check `#faq` — most common issues are answered there\n2. Try a hard refresh (`Cmd+Shift+R` or `Ctrl+Shift+R`)\n3. Try in an incognito window\n4. Check if it\'s a known issue in `#announcements`',
          },
          {
            name: 'When you ask, include',
            value: '• What you\'re trying to do\n• What\'s happening instead\n• Your browser and OS\n• Whether you\'re in demo mode or signed in',
          },
          {
            name: 'Response time',
            value: 'Community: usually fast. Team: within 24 hours on weekdays.',
          },
        ],
    ...
    embeds: [
      {
        ...
        fields: [
          {
            name: 'IBKR — Flex Query (recommended)',
          ...
          {
            name: 'Other brokers',
            value: 'Plaid integration (automatic sync) is coming and will support 12,000+ institutions. Until then, manual CSV from your broker\'s export tool may work — try it and report results in this channel.',
          },
          {
            name: 'Common issues',
            value: '**"No positions found"** — make sure Positions section is included in your Flex Query\n**"Parse error"** — try re-exporting with UTF-8 encoding\n**Missing tickers** — Canadian stocks may need `.TO` suffix',
          },
        ],
        footer: { text: 'Full instructions at unifolio.pro/instructions' },
      },
    ],
  },
    ...
    embeds: [
      {
        ...
        footer: { text: 'Pro pricing will be announced here first.' },
      },
    ],
  },
...
client.once('ready', async () => {
  ...
  for (const [channelName, content] of Object.entries(messages)) {
    const channel = channels.find(c => c.name === channelName && c.isTextBased());
    if (!channel) {
    ...
    try {
      ...
      console.log(`  ✓ #${channelName} — posted + pinned`);
    } catch (e) {
      console.log(`  ✗ #${channelName} — error: ${e.message}`);
    }
  }
