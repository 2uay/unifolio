...
TOTP-based MFA is **deployed in production** via Supabase Auth's built-in `auth.mfa.enroll()` / `auth.mfa.challenge()` / `auth.mfa.verify()` flow. Users enroll an authenticator-app factor from **Settings → Security & Privacy → Two-Factor Authentication**; on sign-in, accounts with a verified factor are prompted for a 6-digit code before the session is upgraded to AAL2. **Accounts created on or after 2026-09-30 must enroll at least one verified TOTP factor before they can connect a Plaid Item** (the in-app Plaid Connect button renders a "set up 2FA" gate until enrollment is complete). Accounts created before that cutover are grandfathered to avoid stranding active brokerage connections, and those users are strongly encouraged to enroll voluntarily from the same Settings panel. The previously-listed compensating controls — email-verification gating, password-length minimum + rate-limited attempts, Postgres RLS on sensitive tables, and the read-only nature of Unifolio (no money movement is possible through the app) — remain in force as defence-in-depth. Enrollment, unenrollment, and challenge results (success / failure) are written to the application audit log (`audit_log`). See **CONSUMER_MFA_RATIONALE.md** for the updated Plaid attestation statement.
